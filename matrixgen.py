...
# to actually generate the predictions. The user needs to enter a value LAMBDA, which
# is baked into the data generated by this file. The code will generate a Directory based
# on the lambda value, and three data files. For example, for a value of LAMBDA = 0.1, 
# the code will generate a directory L0.1/ and the files  L0.1/labels.txt, L0.1/matrix.npy,
# L0.1/cvector.npy. Giving the same LAMBDA to the module distributiongen.py will look in that
# directory, perform the matrix multiplication and output the top 50 gene predictions.
# 
...
#
# The goal is to assign each node a value between 0 and 1, with positively labeled nodes
# always given a 1. Then we'd like the values to be smooth, i.e. the closer two nodes are,
# the closer their values should be. Ultimately this results in a graph where every nodes
# value is just the average of the nodes around it. The method for finding this final 
# distribution is to pick some arbitrary initial distribution and iteratively calculate
# the average of each nodes neighbors until the system converges.
#
